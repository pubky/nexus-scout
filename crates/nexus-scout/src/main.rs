...
//! Config is resolved from the environment; a non-zero exit code signals a startup
//! failure.
...
async fn main() -> ExitCode {
    // Default to nexus_scout=info,warn so operational logs show by default; RUST_LOG overrides.
...
async fn run(cli: Cli) -> Result<(), Error> {
    match cli.command {
        Command::Serve(args) => {
            // Resolve config before starting so a misconfiguration fails closed naming the variable.
