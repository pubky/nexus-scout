//! Model Context Protocol server (stdio). Exposes `query_cypher` and `get_schema`
//! as MCP tools forwarding to the shared [`Scout`] core.
...
/// The MCP server handler wrapping the gateway core.
#[derive(Clone)]
struct ScoutServer {
    scout: Scout,
...
impl ScoutServer {
    ...
    async fn query_cypher(
        ...
        match self.scout.query(&cypher, params, limit).await {
            ...
            // A rejected/timed-out query is a normal tool outcome, returned as an error-flagged result, not a protocol error.
            Err(err) => {
...
/// Runs the MCP server over stdio until the client disconnects. Applies the same
/// production cost-bound gate as HTTP.
///
/// # Errors
///
/// Returns [`Error`] if the gateway cannot connect, the cost-bound gate fails, or
/// the transport fails.
