...
//! Gated behind `integration` + `http`. The key case is the write-rejection
//! parity test: writes must be refused at the HTTP boundary exactly as on the
//! CLI/MCP paths, leaving the graph untouched.
//!
//! Run with a database up and the reader user provisioned:
