//! Wire-contract tests: the JSON nexus-scout emits must match the spec (§5.1)
//! shapes exactly. Comparisons are structural (`serde_json::Value`), since row
//! column order from Neo4j is not significant and is sorted for determinism.
...
fn schema_matches_golden_and_spec_shape() {
    ...
    // Node properties are objects; relationship properties are bare type strings
    // (the spec's deliberate asymmetry).
    let user = v["nodes"]
        .as_array()
        .unwrap()
        .iter()
