...
/// An independent reference scanner: does `s` contain a denied keyword in code
/// position? Shares no code with `unicode::validate`/`tokenizer::lex`, so a
/// divergence between them is caught by the differential property below.
...
proptest! {
    ...
    // Safety invariant: no accepted query may contain a denied keyword in code
    // position, per an independent reference scanner.
    #[test]
