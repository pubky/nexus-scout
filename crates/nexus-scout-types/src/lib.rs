//! Shared wire contract for nexus-scout.
//!
//! The server ([`nexus-scout`](../nexus_scout/index.html)) and the client
//! (`nexus-scout-cli`) both depend on this crate, so the request/response DTOs
//! and the [`ErrorCode`] → HTTP-status / exit-code maps live in exactly one
//! place and are enforced by the compiler rather than duplicated and kept in
//! sync by a test.
#![deny(missing_docs)]
...
///
/// `#[non_exhaustive]` on the wire, but matched exhaustively *within this crate*
/// (in [`http_status`]/[`exit_code`]) so adding a variant is a compile error
/// until both maps are updated.
...
impl ErrorCode {
    /// The stable `SCREAMING_SNAKE_CASE` wire string. The single source of truth
    /// for `Serialize`, `Display`, and [`ErrorCode::from_wire`].
    #[must_use]
    ...
    /// client tolerates a newer server's unknown code instead of failing.
    #[must_use]
...
/// The HTTP status an [`ErrorCode`] maps to. Exhaustive: a new code will not
...
/// The process exit code the `scout` client returns for an [`ErrorCode`]:
/// `0` ok, `1` internal/transient, `2` rejected or bad query, `3` timeout.
/// Exhaustive (see [`http_status`]).
#[must_use]
...
/// The body of `POST /v1/query`. Lenient (unknown fields are ignored) so the
/// versioned API can add optional fields without breaking existing clients.
...
///
/// `count` always equals `results.len()`; `truncated` is `true` when the row or
/// byte cap capped the result.
